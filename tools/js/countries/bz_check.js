...
window.TIN_COUNTRIES['BZ'] = {
  ...
  tin_types: {
    ...
    Individual: {
      name: "TIN",
      format: "NNNNNNN",
      description: "Individuals in Belize are issued a Tax Identification Number (TIN) by the Belize Tax Service for tax administration purposes.\n" +
        "• Issuance: Assigned to taxpayers on registration with the Belize Tax Service.\n\n" +
        ...
        "Validation Scope\n" +
        "This check verifies that the input follows the Belizean 7-digit structural format.\n" +
      ...
      validate(tin) {
        ...
        if (!/^(?!([0-9])\1{6})\d{7}$/.test(tin)) {
          return { valid: false, message: "Does not match the Belizean Individual TIN format." };
        }
    ...
    Entity: {
      name: "TIN",
      format: "NNNNNNN",
      description: "Legal entities in Belize are issued a Tax Identification Number (TIN) by the Belize Tax Service for tax administration purposes.\n" +
        "• Issuance: Assigned to businesses and other entities on registration with the Belize Tax Service.\n\n" +
      ...
      validate(tin) {
        ...
        if (!/^(?!([0-9])\1{6})\d{7}$/.test(tin)) {
          return { valid: false, message: "Does not match the Belizean Entity TIN format." };
        }
        return { valid: true, message: "Valid Tax Identification Number." };
      }
    }
